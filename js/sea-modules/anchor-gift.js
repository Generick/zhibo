	...
	module.exports = {
		...
		init : function() {
            ...
            $(".giftHeader").on('click','.redot',function(){
            	var gid='';
                $.ajax({type: "POST",url: '/ajax/giftClick.php',data: '',dataType: "json",timeout: 1200000,
            ...
            /**礼物，鼠标跟随*/
            $(".giftLists  li:not(:first)").mouseover(function(){
                var price_html;
                ...
                if($(this).find("img").attr("join")>0){
                    $('.gift-tip-popup').find(".gift-tip-info").html('<div class="gift-tip-pic"><img alt="'+$(this).find(".gfname").html()+'" src="'+$(this).find("img").attr("src")+'" class="'+$(this).find("img").attr("class")+'"/></div>\
                		<div class="gift-tip-detail"><p  class="gp1">'+$(this).find(".gfname").html()+'<i>'+price_html+'</p><p class="gp2">连送<i>2</i>组<i> '+$(this).find("img").attr("join")+'</i>，触发连击特效</p></div>');
                }else{
                    $('.gift-tip-popup').find(".gift-tip-info").html('<div class="gift-tip-pic"><img alt="'+$(this).find(".gfname").html()+'" src="'+$(this).find("img").attr("src")+'" class="'+$(this).find("img").attr("class")+'"/></div><div class="gift-tip-detail"><p  class="gp1">'+$(this).find(".gfname").html()+'<i>'+price_html+'</p></div>')
                }
            ...
            if(UIF.handler.login){
                 ...
                 $(".giftLists  li:first").mouseover(function(){
                     $('.gift-tip-popup').find(".gift-tip-info").html('<div class="gift-tip-pic"><img alt="'+$(this).find(".gfname").html()+'" src="'+$(this).find("img").attr("src")+'" class="'+$(this).find("img").attr("class")+'"/></div>\
                     		<div class="gift-tip-detail"><p  class="gp1">'+$(this).find(".gfname").html()+'<i> '+$(this).find(".first-nus").text()+'</i> 个</p>' +
                     '<p class="gp22"><i> '+fi_time+'</i>秒获取下一颗</p></div>');
			...
			$(".giftHeader span").click(function() {
                ...
                $(".xGiftList").css({"width" : this.oWidth,"margin-left":0});
			});
			/** 礼物选中 */
		...
		speGift : function(data){
			swf.spe({nickname : data.nickname,speimg : "/static_data/images_css/upgrades/pic_consumelevel_"+data.splev+".png"});
		},
		enterCar : function(data) {
			if (data.enterCar != null) {
				...
				if(data.nickname == UIF.currentUserNickname){
					setTimeout(function(){
						swf.fil({giftId : enter.giftId,number : enter.number,car : 1,nickname : data.nickname,carName : carName, needAddFlag: needAddFlag});
					},3 * 1000);
				}else{
					swf.fil({giftId : enter.giftId,number : enter.number,car : 1,nickname : data.nickname,carName : carName, needAddFlag: needAddFlag});
				}
