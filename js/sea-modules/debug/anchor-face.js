	...
	module.exports = {
		...
		getFaceGIFByCode : function(code, lazy) {
			...
			for (var i = 0; i < base.FaceData.length; i++) {
				...
				if (arr[1] == code || arr[0] == code) {
					if (lazy) {
						return '<img class="lazy" src="' + UIF.cdn_domain + '/img/pixel.gif" data-original="/img/face/' + arr[0] + '" alt="' + arr[1] + '" title="' + arr[1] + '">';
					} else {
						return '<img src="' + UIF.cdn_domain + '/img/face/' + arr[0] + '" alt="' + arr[1] + '" title="' + arr[1] + '">';
					}
				}
			}
		},
		faceReplaceImg : function(str, lazy) {
			...
			try {
				...
				if (lazy) {
					src = src.replace(/@(\d{1,2})@/g, '<img class="lazy" src="' + UIF.cdn_domain + '/img/pixel.gif" data-original="/img/ribbon/$1.gif" />');
				} else {
					src = src.replace(/@(\d{1,2})@/g, '<img src="' + UIF.cdn_domain + '/img/ribbon/$1.gif"/>');
				}
			} catch (e) {
				UIF.log(e);
			}
		...
		initFaceType : function(type) {
			...
			for (var i = sIdx; i < len; i++) {
				...
				var img = (i > eIdx) ? '' : '<img src="' + UIF.cdn_domain + '/img/face/' + base.FaceData[i][0] + '" alt="' + base.FaceData[i][1] + '" title="' + base.FaceData[i][1] + '">';
				faceHtml += '<td>' + img + '</td>';
				if (0 == (i + 1) % 10) {
					faceHtml += '</tr>';
				}
